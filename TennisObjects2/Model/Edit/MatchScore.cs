...
{
	public class MatchScore : BusinessBase
	{
		...
		/// </remarks> 
		public class MatchSet : BusinessBase
		{
			...
			public int SetNumber { get { return _setNumber; } }
			...
			//public bool PreviousSetPlayed { get { return PreviousSet.SetPlayed; } }
		...
		public override BrokenRulesCollection BrokenRulesCollection
		{
			get
			{
				...
				for (int i = 0; i <= 4; i++)
				{
					foreach (BrokenRule rule in _Sets[i].BrokenRulesCollection)
						rules.Add(rule);
				}
				return rules;
			}
		...
		internal void Match_Saved(object sender, EventArgs e)
		{
			MarkOld();
		}
