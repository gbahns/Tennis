using System;
using Bahns.Properties;
using System.Text.RegularExpressions;
using System.Reflection;

namespace Bahns.Util.Validation
{
	/// <summary>
	...
	public static class CommonRules
	{
		...
		public static bool StringRequired(object target, RuleArgs e)
		{
			...
			{
				e.Description = string.Format(Resources.StringRequiredRule, e.PropertyDisplayName);
				return false;
			}
			return true;
		}
		...
		public class MaxLengthRuleArgs : RuleArgs
		{
			...
			/// <param name="propertyName">Name of the property to validate.</param>
			/// <param name="maxLength">Max length of characters allowed.</param>
			public MaxLengthRuleArgs(string propertyName, string propertyDisplayName, int maxLength)
				: base(propertyName, propertyDisplayName)
			{
				_maxLength = maxLength;
			}
		...
		public class IntegerMaxValueRuleArgs : RuleArgs
		{
			...
			/// <param name="propertyName">Name of the property.</param>
			/// <param name="maxValue">Maximum allowed value for the property.</param>
			public IntegerMaxValueRuleArgs(string propertyName, string propertyDisplayName, int maxValue)
				: base(propertyName, propertyDisplayName)
			{
				_maxValue = maxValue;
			}
		...
		public class IntegerMinValueRuleArgs : RuleArgs
		{
			...
			/// <param name="propertyName">Name of the property.</param>
			/// <param name="minValue">Minimum allowed value for the property.</param>
			public IntegerMinValueRuleArgs(string propertyName, string propertyDisplayName, int minValue)
				: base(propertyName, propertyDisplayName)
			{
				_minValue = minValue;
			}
		...
		public class MaxValueRuleArgs<T> : RuleArgs
		{
			...
			/// <param name="propertyName">Name of the property.</param>
			/// <param name="maxValue">Maximum allowed value for the property.</param>
			public MaxValueRuleArgs(string propertyName, string propertyDisplayName, T maxValue)
				: base(propertyName, propertyDisplayName)
			{
				_maxValue = maxValue;
			}
		...
		public class MinValueRuleArgs<T> : RuleArgs
		{
			...
			/// <param name="propertyName">Name of the property.</param>
			/// <param name="minValue">Minimum allowed value for the property.</param>
			public MinValueRuleArgs(string propertyName, string propertyDisplayName, T minValue)
				: base(propertyName, propertyDisplayName)
			{
				_minValue = minValue;
			}
		...
		public static bool RegExMatch(object target, RuleArgs e)
		{
			...
			if (!rx.IsMatch(Utilities.CallByName(target, e.PropertyName, CallType.Get).ToString()))
			{
				switch (rx.ToString())
				{
					case RegExPatterns.Filename:
						e.Description = string.Format("{0} contains invalid characters", e.PropertyDisplayName);
						break;
					default:
						e.Description = String.Format(Resources.RegExMatchRule, e.PropertyDisplayName);
						break;
				}
				return false;
			}
		...
		public class RegExRuleArgs : RuleArgs
		{
			...
			//public RegExRuleArgs(string propertyName, RegExPatternsEnum pattern)
			//    :
			//  base(propertyName)
			//{
			...
			//public static string GetPattern(RegExPatternsEnum pattern)
			//{
			//    switch (pattern)
			//    {
			//        case RegExPatternsEnum.SSN:
			//            return @"^\d{3}-\d{2}-\d{4}$";
			//        case RegExPatternsEnum.Email:
			//            return @"^[A-Za-z0-9._%-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,4}$";
			//        case RegExPatternsEnum.Filename:
			//            return @"^[^\\/:*?""<>|]*$";
			//        default:
			//            return string.Empty;
			//    }
